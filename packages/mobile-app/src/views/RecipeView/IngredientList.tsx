...
export const IngredientList: React.FC<IngredientListProps> = observer(
    ({ multiplier }) => {
        ...
        return (
            <Ingredients>
                {currentRecipe.recipe.ingredientSections?.flatMap(
                    (section, si) => [
                        (currentRecipe.recipe?.ingredientSections?.length ??
                            0) > 1 &&
                            section.name && (
                                <SectionTitle
                                    isFirstChild={si === 0}
                        ...
                        ...(section.recipeIngredients?.map(rcpIngr => (
                            <IngredientRow key={`${section.id}_${rcpIngr.id}`}>
                                <IngredientText>
                                    •{' '}
                                    {capitalize(rcpIngr.ingredient?.name || '')}{' '}
                                </IngredientText>
                                <IngredientText>
                                    {round(
                                        rcpIngr.quantityFrom * multiplier,
                                        2,
                                    )}
                                    {rcpIngr.quantityTo &&
                                        `—${round(
                                            rcpIngr.quantityTo * multiplier,
                                            2,
                                        )}`}{' '}
                                    {rcpIngr.unit?.name}
                                </IngredientText>
