...
export const StepsList: React.FC<StepsListProps> = observer(
    ({ currentStep, currentSection, onChildrenLayout }) => {
        ...
        return (
            <View>
                {currentRecipe.recipe.sections?.flatMap(
                    (section, sectionIndex) => [
                        (currentRecipe.recipe?.sections?.length ?? 0) > 1 &&
                            section.name && (
                                <SectionTitle
                        ...
                        ...(section.recipeSteps?.map((step, i) => (
                            ...
                            >
                                <RecipeStepLineDecorator
                                    ...
                                    isLast={
                                        i ===
                                        (section.recipeSteps?.length ?? 1) - 1
                                    }
                                />
                                <RecipeStepText
                                    isCurrent={
