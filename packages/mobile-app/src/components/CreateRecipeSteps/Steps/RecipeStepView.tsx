...
const RecipeStepPreview: React.FC<RecipeStepPreviewProps> = observer(
    ({ step, drag }) => {
...
const SectionSeparatorView: React.FC<SectionSeparatorViewProps> = observer(
    ({ section }) => {
        const { draftRecipe } = useRootStore();

        const prevSectionIndex =
            (draftRecipe.recipe.sections?.findIndex(s => s.id === section.id) ??
                0) - 1;
        ...
        return (
            <>
                {prevSectionIndex >= 0 && draftRecipe.recipe.sections && (
                    <AddStepButton
                        targetSectionId={
                            draftRecipe.recipe.sections[prevSectionIndex].id
                        }
                    />
                )}
