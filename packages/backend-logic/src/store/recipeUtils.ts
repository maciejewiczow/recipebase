...
export const removeEmptyIngredientsAndSteps = (recipe: Recipe) => {
    for (const ingredientSection of recipe.ingredientSections ?? []) {
        ingredientSection.recipeIngredients = ingredientSection.recipeIngredients
            ?.filter(ri => !!ri.ingredient?.name?.trim());
    }

    for (const section of recipe.sections ?? []) {
        section.recipeSteps = section.recipeSteps
        ...
        for (const ingredientSection of recipe.ingredientSections ?? []) {
            for (const recipeIngredient of ingredientSection.recipeIngredients ?? []) {
                if (recipeIngredient.ingredient)
                    promises.push(manager.getRepository(Ingredient).save(recipeIngredient.ingredient));

                if (recipeIngredient.unit)
                    promises.push(manager.getRepository(Unit).save(recipeIngredient.unit));

                promises.push(manager.getRepository(RecipeIngredient).save(recipeIngredient));
            }
            promises.push(manager.getRepository(IngredientSection).save(ingredientSection));
        }

        for (const recipeSection of recipe.sections ?? []) {
            promises.push(manager.getRepository(RecipeStep).save(recipeSection.recipeSteps ?? []));
            promises.push(manager.getRepository(RecipeSection).save(recipeSection));
        }
